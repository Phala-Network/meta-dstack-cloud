...
- `dstack.rootfs_dev` (optional) overrides the default `/dev/vda` block device that stores the verity data and hash. Set this when the root filesystem lives on a different disk or naming scheme (for example `/dev/sdb` or `/dev/disk/by-id/...`).
